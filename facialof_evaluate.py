...
def preprocess_flow(image, flow, label, Resize, Resize_2, crop_dims, first_flow):
    ...
    if first_flow:
        r_min = np.min(y_lm); r_max = np.max(y_lm); c_min = np.min(x_lm); c_max = np.max(x_lm)
    else:
...
def dispOpticalFlow( Image,Flow,Divisor,name,scale,thick,colors,show):
    ...
    for i in range(1, Imax):
        for j in range(1, Jmax):
            X1 = (i)*Divisor
            Y1 = (j)*Divisor
            X2 = int(X1 + s*Flow[X1,Y1,1])
            Y2 = int(Y1 + s*Flow[X1,Y1,0])
            X2 = np.clip(X2, 0, PictureShape[0])
            Y2 = np.clip(Y2, 0, PictureShape[1])
            #add all the lines to the mask
            mask = cv2.arrowedLine(mask, (Y1,X1),(int(Y2),int(X2)), colors, thick)
...
def flow_viz_arrows(flow, image, divider, title, scale, processed, thick,colors, choose_scale = 0):
    if processed:
        flow = rotate_flow_2(flow)
    image = Resize_image(image, flow.shape[0:2], image.shape[0:2], 0)
    if scale:
        sc = 6
        if choose_scale:
            sc = choose_scale
        dispOpticalFlow(image, flow, divider, title, sc * divider, thick, colors, 0)
    return []

def save_ground_truth(x1, x2, resultpath, label):
    im_path_1 = resultpath + label +'_input' + '1' + '.png'
    im_path_2 = resultpath + label +'_input' +  '2' + '.png'
    save_img(im_path_1, x1)
    save_img(im_path_2, x2)

# Compute endpoint and angular errors during evaluation
def average_endpoint_error_compute(gt, preds):
    diff = gt - preds
    total_sum = 0
    for i in range(diff.shape[0]):
        for j in range(diff.shape[1]):
            total_sum += np.linalg.norm(diff[i, j, :], 2)
    sqrt_sum_diff = total_sum
    mean_sqrt_sum_diff = total_sum / (diff.shape[0] * diff.shape[1])
    return sqrt_sum_diff, mean_sqrt_sum_diff

def average_angular_error_compute(gt, preds):
    try:
        B, H, W, _ = gt.shape
        gt = np.concatenate((gt, np.ones((B, H, W, 1))), axis=3)
        preds = np.concatenate((preds, np.ones((B, H, W, 1))), axis=3)
    except:
        H, W, _ = gt.shape
        gt = np.concatenate((gt, np.ones((H, W, 1))), axis=2)
        preds = np.concatenate((preds, np.ones((H, W, 1))), axis=2)
    total_sum = 0
    for i in range(H):
        for j in range(W):
            total_sum += np.arctan2(np.linalg.norm(np.cross(gt[i, j, :], preds[i, j, :]), 2), np.dot(gt[i, j, :], preds[i, j, :]))
    total_sum /= (H * W)
    return total_sum

# Function to save predictions and ground truth flows
def save_preds(resultpath, label, flow_pred, gt, error_txt_file, error_txt_file_angular, gt_resize):
    flo_path, gtimg_path =  resultpath + label + '_pred0', resultpath + label + '_gt.png'
    if OF_MODEL == "FacialOF":
        origs = [gt.shape[0], gt.shape[1]]
        gt_resize = gt
        flow_pred = rotate_flow_2(flow_pred)
    else:
        origs = [gt_resize[0], gt_resize[1]]
        resize = [flow_pred.shape[0], flow_pred.shape[1]]
        if (np.asarray(gt).shape[0] > resize[0]) and (np.asarray(gt).shape[1] > resize[1]):
            gt_resize = Resize_image(gt, resize, origs = origs, im_or_flow = 1)
        else:
            flow_pred = Resize_image(flow_pred, origs, origs = resize, im_or_flow = 1)
            gt_resize = gt
        flow_pred = rotate_flow_2(flow_pred)
    save_img(gtimg_path, computeImg(gt_resize))
    save_img(flo_path + '.png', computeImg(flow_pred))

    error, error_mean = average_endpoint_error_compute(gt_resize, flow_pred)
    error_angular = average_angular_error_compute(gt_resize, flow_pred)
    error_file = open(error_txt_file, "a")
    error_file.write('\n')
    error_file.write(label + ' error: ' + str(error) + ',\t average error: ' + str(error_mean))
    error_file.close()
    error_file_angular = open(error_txt_file_angular, "a")
    error_file_angular.write('\n')
    error_file_angular.write(label + ' average angular error: ' + str(error_angular))
    error_file_angular.close()
...
def farneback_flow(x1, x2):
    x1, x2 = convert(x1, 0, 255, np.uint8), convert(x2, 0, 255, np.uint8)
    x1, x2 = cv2.cvtColor(x1, cv2.COLOR_RGB2GRAY), cv2.cvtColor(x2, cv2.COLOR_RGB2GRAY)
    flow = cv2.calcOpticalFlowFarneback(x1, x2, None, 0.5, 3, 15, 3, 5, 1.2, 0)
    return flow
...
def tvl1_flow(x1, x2):
    ...
    flow = optical_flow.calc(x1, x2, None)  
    return flow

def get_test_inputs(X1, file_list, Resize, Resize_info, n):
    im1 = np.zeros((2, Resize[0], Resize[1], 3)).astype('float')
    im2 = np.zeros((2, Resize[0], Resize[1], 3)).astype('float')
    flow = np.zeros((2, Resize_info[0][1], Resize_info[0][2], 2)).astype('float')

    input_11 = get_input(file_list + '/' + X1[n] + '.jpg')
    input_12 = get_input(file_list + '/' + X1[n+1] + '.jpg')
    flow_1 = read_flow(file_list + '/' + X1[n] + '.flo')

    input_21 = get_input(file_list + '/' + X1[n+1] + '.jpg')
    input_22 = get_input(file_list + '/' + X1[n+2] + '.jpg')
    flow_2 = read_flow(file_list + '/' + X1[n+1] + '.flo')

    gt_temp_1, gt_temp_2 = flow_1, flow_2
    y1, x11, crop_dims = preprocess_flow(image = input_11, flow = gt_temp_1, 
                                        label = X1[n], Resize = Resize, Resize_2 = Resize_info[0][1:3], 
                                        crop_dims = [], first_flow = 1)
    x12 = preprocess_flow(image = input_12, flow = gt_temp_1, 
                            label = X1[n], Resize = Resize, Resize_2 = Resize_info[0][1:3], 
                            crop_dims = crop_dims, first_flow = 0)
    y2, x21, crop_dims = preprocess_flow(image = input_21, flow = gt_temp_2, 
                                        label = X1[n + 1], Resize = Resize, Resize_2 = Resize_info[0][1:3],
                                        crop_dims = [], first_flow = 1)
    x22 = preprocess_flow(image = input_22, flow = gt_temp_2, 
                        label = X1[n], Resize = Resize, Resize_2 = Resize_info[0][1:3], 
                        crop_dims = crop_dims, first_flow = 0)
    im1[0], im1[1], im2[0], im2[1] = x11, x21, x12, x22
    flow[0], flow[1] = y1, y2
    return X1, im1, im2, flow, n, n+1
...
if __name__ == '__main__':
    ...
    # Specify the method of computing optical flow for evaluation.
    # "FacialOF" will use our method, "farneback" for Gunnar-Farneback flow, "tvl1" for TVL1 flow
    # For liteflownet, flownet3, or flownet2, specify flownet3path, flownet2path, or liteflownetpath to where the results
    # are stored.
    ...
    with tf.device(device_use):
        ...
        x1, x2, gt  = [],[],[]
        X_test_all_all = [ims.split('.')[0] for ims in sorted(os.listdir(file_list_test)) if ims.endswith('.jpg')]
        N_0, N_1 = 0, 32000
        N_0, N_1 = max(0, N_0), min(N_1, len(X_test_all_all))
        X_test_all = X_test_all_all[N_0:N_1]
        N_testing = len(X_test_all) - 2
        
        error_txt_file, angular_error_txt_file = results_path + 'endpoint_errors.txt', results_path + 'angular_errors.txt'

        with open(error_txt_file, 'w') as f:
            f.write('Average endpoint errors:')
            f.close
        with open(angular_error_txt_file, 'w') as f:
            f.write('Average angular errors:')
            f.close
        
        flow_pred = None
        
        for n, t in enumerate(range(0, N_testing, 2)):
            n = t
            if n > 0:
                n = t
            if not np.mod(n, 2):
                ...
                if n:
                    n_1 += 2
                X_test_all, x1, x2, gt, n_1, n_2 = get_test_inputs(X_test_all, file_list_test, Resize, resize_data, n)
            if OF_MODEL == "FacialOF":
                flow_pred = decoderModel.predict([x1, x2])
                y1, X21 = flow_pred[0][0], flow_pred[-1][0]
                y2, X22 = flow_pred[0][1], flow_pred[-1][1]
                save_img(results_path + X_test_all[n_1] + '_deformed.png', X21)
                save_img(results_path + X_test_all[n_2] + '_deformed.png', X22)
            elif OF_MODEL == "farneback":
                y1, y2 = farneback_flow(x1[0], x2[0]), farneback_flow(x1[1], x2[1])
                X21, X22 = [], []
            elif OF_MODEL == "tvl1":
                y1, y2 = tvl1_flow(x1[0], x2[0]), tvl1_flow(x1[1], x2[1])
                X21, X22 = [], []
            # For other OF_MODELS (liteflownet, pwc-net, flownet2, flownet3), modify the below based on where you saved 
            # your computed flows
            elif OF_MODEL == "flownet3":
                y1, y2 = read_flow(flownet3path + 'face_results/' + X_test_all[n] + '/flow[0].fwd.flo'), read_flow(flownet3path + 'face_results/' + X_test_all[n + 1] + '/flow[0].fwd.flo')
                X21, X22 = [], []
            elif OF_MODEL == "liteflownet":
                y1, y2 = read_flow(liteflownetpath + X_test_all[n] + '.flo'), read_flow(liteflownetpath + X_test_all[n + 1] + '.flo')
                X21, X22 = [], []
            elif OF_MODEL == "flownet2":
                y1, y2 = read_flow(flownet2path + 'face_results/' + X_test_all[n] + '.flo'), read_flow(flownet2path + 'face_results/' + X_test_all[n+1] + '.flo')
                X21, X22 = [], []
            save_preds(results_path, X_test_all[n_1], y1, gt[0], 
                                        error_txt_file, angular_error_txt_file, resize_data[0][1:3])
            save_preds(results_path, X_test_all[n_2], y2, gt[1], 
                                        error_txt_file, angular_error_txt_file, resize_data[0][1:3])